...
from graphiti_core.helpers import parse_db_date, spread_sanitized_attributes
...
class EntityEdge(Edge):
    ...
    async def save(self, driver: GraphDriver):
        ...
        if driver.provider == GraphProvider.KUZU:
            ...
            spread_sanitized_attributes(edge_data, self.attributes, uuid=self.uuid, kind='edge')
            result = await driver.execute_query(
                get_entity_edge_save_query(driver.provider),
                edge_data=edge_data,
            )
